...
| Signal | State |
|---|---|
| Backend suite | **154 passed** (`uv run pytest tests/`) |
...
- **Stale packaging metadata.** `pyproject.toml` still carries the pre-rename package name `devteam-ai` and placeholder `your-repo` project URLs, and lists LangChain dependencies the retired orchestrator no longer needs.
...
2. Clean the packaging metadata (`devteam-ai` → `appforge`, real repository URLs, drop unused LangChain deps).
3. Harden the web bridge for multi-user / reconnect durability.
4. Refresh or archive the LangGraph-era design docs so `docs/` matches the shipped engine.
