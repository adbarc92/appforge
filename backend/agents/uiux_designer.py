"""Real UI/UX Designer agent backed by an LLM chat model with structured output.

The agent wraps an arbitrary `BaseChatModel` (Anthropic by default) and turns a
PRD into a structured design spec: a set of design tokens plus a list of
components. Like the Tech Lead, it returns JSON parsed into a Pydantic model.
The retry path mirrors `tech_lead.py`: retry once on malformed JSON; on a second
malformed response, return an error dict.

The output shape mirrors the other Phase 3/4 agents:

    {"status": "success", "artifact": {"design_spec": {...}}, "cost": 0.0}
...
class DesignSpecResponse(BaseModel):
    """Structured output expected from the LLM: design tokens + components."""

    tokens: dict[str, Any] = Field(default_factory=dict)
    components: list[dict[str, Any]] = Field(default_factory=list)


class UiuxDesignerAgent(InstrumentedAgent):
    """Real UI/UX designer agent that turns a PRD into a design spec."""

    def __init__(
        self,
        model: BaseChatModel | None = None,
        name: str = "uiux_designer",
        ...
        (`UiuxDesignerAgent(model=...)`) and the registry form
        (`UiuxDesignerAgent(name=..., emit_callback=..., config=...)`).
        """
    ...
    async def execute(  # type: ignore[override]
        ...
        return {
            "status": "success",
            "artifact": {"design_spec": parsed.model_dump()},
            "cost": 0.0,
        }

    async def _call_with_retry(self, prompt: str) -> DesignSpecResponse | None:
        """Call the model up to twice, retrying once on malformed JSON."""
        for attempt in (1, 2):
            messages = [
                SystemMessage(
                    content=(
                        "You are a product designer producing a structured UI "
                        "design spec. Respond ONLY with JSON."
                    )
                ),
                HumanMessage(
                    content=(
    ...
    @staticmethod
    def _parse(text: str) -> DesignSpecResponse | None:
        """Parse a JSON LLM response into a `DesignSpecResponse`, or None.
        ...
        try:
            ...
            return DesignSpecResponse.model_validate(data)
